...
import { LyricsManager, openLyricsPanel } from './lyrics.js';
...
import { initializeCasting } from './app/casting.js';
import { initializeKeyboardShortcuts, showKeyboardShortcuts, showCustomizeShortcutsModal } from './app/keyboard.js';
import {
    showOfflineNotification,
    hideOfflineNotification,
    showUpdateNotification,
    showMissingTracksNotification,
} from './app/notifications.js';
import { showDiscographyDownloadModal } from './app/modals.js';
import {
    uploadCoverImage,
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.import-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            const vkFileInput = document.getElementById('vk-file-input');
            if (vkFileInput) {
                vkFileInput.value = importType === 'vk' ? document.getElementById('vk-file-input').value : '';
            }
        });
    });

    // CSV Import - Guide switching (Spotify, Apple, YTM, Yandex, VK)
    const csvGuidePanels = {
        spotify: { panel: document.getElementById('csv-spotify-guide'), btn: document.getElementById('csv-spotify-btn') },
        apple: { panel: document.getElementById('csv-apple-guide'), btn: document.getElementById('csv-apple-btn') },
        ytm: { panel: document.getElementById('csv-ytm-guide'), btn: document.getElementById('csv-ytm-btn') },
        yandex: { panel: document.getElementById('csv-yandex-guide'), btn: document.getElementById('csv-yandex-btn') },
        vk: { panel: document.getElementById('csv-vk-guide'), btn: document.getElementById('csv-vk-btn') },
    };

    const csvFileContainer = document.getElementById('csv-input-container');
    const yandexFileContainer = document.getElementById('yandex-file-input-container');
    const vkFileContainer = document.getElementById('vk-file-input-container');

    Object.entries(csvGuidePanels).forEach(([key, { panel, btn }]) => {
        if (!btn || !panel) return;
        
        btn.addEventListener('click', () => {
            // Hide all panels and reset buttons
            Object.values(csvGuidePanels).forEach(({ panel: p, btn: b }) => {
            ...
            if (yandexFileContainer) yandexFileContainer.style.display = 'none';
            if (vkFileContainer) vkFileContainer.style.display = 'none';
            ...
            // Show file input for Yandex and VK
            if (key === 'yandex' && yandexFileContainer) {
                yandexFileContainer.style.display = 'block';
            }
            if (key === 'vk' && vkFileContainer) {
                vkFileContainer.style.display = 'block';
            }
        });
    });
    ...
    const yandexBtn = document.getElementById('csv-yandex-btn');
    ...
    const yandexGuide = document.getElementById('csv-yandex-guide');
    const yandexFileInputContainer = document.getElementById('yandex-file-input-container');
    const inputContainer = document.getElementById('csv-input-container');

    // Yandex parser script
    const yandexParserScript = `(function collectAllTracks() {
    ...
    function Tick() {
        if (!scrollContainer) {
            console.error("Scroll container not found. Aborting.");
            return;
        }
        ...
        setTimeout(() => {
            ...
            if (scrollContainer.scrollTop === previousHeight) {
                console.log("Scrolling complete, final check...");
                setTimeout(() => {
    ...
    function finish() {
        ...
        console.log("Downloaded " + trackCount + " unique tracks!");
    ...
    function startWhenReady() {
        ...
        if (scrollContainer) {
            console.log("Scroll container found. Scrolling to top...");
    ...
    console.log("Looking for scroll container and tracks...");
    localStorage.removeItem('music_tracks');
    startWhenReady();
})();`;

    // Yandex copy script button
    const yandexCopyScriptBtn = document.getElementById('yandex-copy-script-btn');
    const yandexFileInput = document.getElementById('yandex-file-input');

    yandexCopyScriptBtn?.addEventListener('click', () => {
        navigator.clipboard.writeText(yandexParserScript).then(() => {
            yandexCopyScriptBtn.textContent = '✅ Script Copied!';
            yandexFileInputContainer.style.display = 'block';
            setTimeout(() => {
                yandexCopyScriptBtn.textContent = '📋 Copy Parser Script';
            }, 3000);
        }).catch(err => {
            alert('Failed to copy script. Please select and copy manually from the source code.');
            console.error('Copy failed:', err);
        });
    });

    yandexFileInput?.addEventListener('change', (e) => {
        const file = e.target.files?.[0];
        if (file) {
            document.getElementById('yandex-status').textContent = `Selected: ${file.name}`;
        }
    });

    // VK Parser Script
    const vkParserScript = `(function() {
    const actionsLines = document.querySelectorAll('.AudioPlaylistSnippet__actions');

    if (actionsLines.length === 0) {
        alert('❌ No VK playlist found!\\n\\nPlease open a playlist page first.\\nGo to vk.com/audios and select a playlist.');
        return;
    }
    ...
    exportBtn.style.backgroundColor = '#4C75A3';
    ...
    // VK copy script button
    const vkCopyScriptBtn = document.getElementById('vk-copy-script-btn');
    const vkFileInput = document.getElementById('vk-file-input');

    vkCopyScriptBtn?.addEventListener('click', () => {
        navigator.clipboard.writeText(vkParserScript).then(() => {
            vkCopyScriptBtn.innerHTML = '✅ Script Copied!';
            vkFileContainer.style.display = 'block';
            setTimeout(() => {
                vkCopyScriptBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style="margin-right: 8px; vertical-align: middle;"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>🐑 Copy VK Parser Script';
            }, 3000);
        }).catch(err => {
    ...
    vkFileInput?.addEventListener('change', (e) => {
        const file = e.target.files?.[0];
        if (file) {
            document.getElementById('vk-status').textContent = `📤 Selected: ${file.name}`;
        }
    });

    if (spotifyBtn && appleBtn && ytmBtn && yandexBtn) {
        spotifyBtn.addEventListener('click', () => {
            ...
            yandexBtn.classList.remove('btn-primary');
            yandexBtn.classList.add('btn-secondary');
            yandexBtn.style.opacity = '0.7';

            spotifyGuide.style.display = 'block';
            appleGuide.style.display = 'none';
            ytmGuide.style.display = 'none';
            yandexGuide.style.display = 'none';
        ...
        yandexBtn.addEventListener('click', () => {
            yandexBtn.classList.remove('btn-secondary');
            yandexBtn.classList.add('btn-primary');
            yandexBtn.style.opacity = '1';
            ...
            yandexGuide.style.display = 'block';
            spotifyGuide.style.display = 'none';
            appleGuide.style.display = 'none';
            ytmGuide.style.display = 'none';
            inputContainer.style.display = 'none';
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#create-playlist-btn')) {
            ...
            if (yandexFileInput) yandexFileInput.value = '';
            if (yandexFileInputContainer) yandexFileInputContainer.style.display = 'none';
            document.getElementById('yandex-status').textContent = '';
            document.getElementById('jspf-file-input').value = '';
            document.getElementById('xspf-file-input').value = '';
            document.getElementById('xml-file-input').value = '';
            document.getElementById('m3u-file-input').value = '';
        ...
        if (e.target.closest('#playlist-modal-save')) {
            ...
            if (name) {
                ...
                if (editingId) {
                    ...
                    const isYandexActive = document.getElementById('csv-yandex-btn')?.classList.contains('btn-primary');
                    const ytmUrlInput = document.getElementById('ytm-url-input');
                    const yandexUrlInput = document.getElementById('yandex-url-input');
                    ...
                    if (isYTMActive && ytmUrlInput.value.trim()) {
                    ...
                    } else if (isYandexActive && yandexFileInput && yandexFileInput.files.length > 0) {
                        // Import from Yandex Music TXT file
                        importSource = 'yandex_import';
                        const file = yandexFileInput.files[0];
                        ...
                        try {
                            ...
                            progressFill.style.width = '0%';
                            progressCurrent.textContent = '0';
                            currentTrackElement.textContent = 'Reading Yandex Music export...';
                            if (currentArtistElement) currentArtistElement.textContent = '';
                            ...
                            if (lines.length === 0) {
                                alert('No tracks found in the file!');
                                progressElement.style.display = 'none';
                                return;
                            }

                            statusEl.textContent = `Found ${lines.length} tracks. Importing...`;
                            progressTotal.textContent = lines.length.toString();

                            // Parse "Artist - Title" format
                            const tracksToSearch = lines.map(line => {
                            ...
                            for (let i = 0; i < tracksToSearch.length; i++) {
                                const { artist, title } = tracksToSearch[i];
                                
                                progressCurrent.textContent = (i + 1).toString();
                                const percentage = ((i + 1) / tracksToSearch.length) * 100;
                                progressFill.style.width = `${percentage}%`;
                                currentTrackElement.textContent = title;
                                if (currentArtistElement) currentArtistElement.textContent = artist;
                                ...
                                try {
                                    ...
                                    if (searchResults.items && searchResults.items.length > 0) {
                                        foundTracks.push(searchResults.items[0]);
                                    } else {
                                        missingTracks.push({ title, artist });
                                    }
                                } catch {
                                    missingTracks.push({ title, artist });
                                }
                            ...
                            tracks = foundTracks;
                            console.log(`Imported ${tracks.length} tracks from Yandex Music`);
                            trackImportCSV(name || 'Untitled', tracks.length, missingTracks.length);
                            ...
                            progressElement.style.display = 'none';
                            alert(`Error importing from Yandex Music: ${err.message}`);
                            return;
                        } finally {
                            setTimeout(() => {
                                progressElement.style.display = 'none';
                    ...
                    } else if (isYandexActive && yandexUrlInput.value.trim()) {
                        // Legacy URL input (deprecated)
                        alert('Please use the file upload method for Yandex Music imports.');
                        return;
                    ...
                    } else if (vkFileInput && vkFileInput.files.length > 0) {
                        // VK Import
                        importSource = 'vk_import';
                        const file = vkFileInput.files[0];
                        const vkStatusDiv = document.getElementById('vk-status');

                        try {
                            const { VKImporter } = await import('./vk-importer.js');
                            const importer = new VKImporter();

                            // Parse file
                            const vkTracks = await importer.processFile(file);

                            if (vkTracks.length === 0) {
                                alert('No tracks found in the file! Please check the format (Artist - Title per line).');
                                return;
                            }
                            ...
                            progressFill.style.width = '0%';
                            progressCurrent.textContent = '0';
                            progressTotal.textContent = vkTracks.length.toString();
                            currentTrackElement.textContent = `Processing ${vkTracks.length} tracks from VK...`;
                            if (currentArtistElement) currentArtistElement.textContent = '';

                            // Import tracks
                            const result = await importer.importPlaylist(
                                vkTracks,
                                api,
                                name || 'VK Import',
                                (current, total, track) => {
                                    ...
                                    if (currentArtistElement) currentArtistElement.textContent = track.artist || '';
                                }
                            );
                            ...
                            if (tracks.length === 0) {
                                alert('No valid tracks found! Make sure the file is in "Artist - Title" format.');
                                if (vkStatusDiv) {
                                    vkStatusDiv.textContent = '❌ Error: No tracks imported';
                                    vkStatusDiv.style.color = '#ff5252';
                                }
                                progressElement.style.display = 'none';
                                return;
                            }

                            console.log(`Imported ${tracks.length} tracks from VK`);

                            if (vkStatusDiv) {
                                const failedCount = result.failed + result.notFound;
                                vkStatusDiv.textContent = `✅ Imported ${result.imported} tracks${failedCount > 0 ? `, ${failedCount} failed` : ''}`;
                                vkStatusDiv.style.color = '#4cb176';
                            }

                            // Show missing tracks notification
                            if (result.failedTracks.length > 0) {
                                setTimeout(() => {
                                    showMissingTracksNotification(result.failedTracks);
                                }, 500);
                            }

                        } catch (error) {
                            console.error('Failed to parse VK file!', error);
                            alert('Failed to parse VK file! ' + error.message);
                            if (vkStatusDiv) {
                                vkStatusDiv.textContent = '❌ Error: ' + error.message;
                                vkStatusDiv.style.color = '#ff5252';
                            }
                            return;
                        } finally {
