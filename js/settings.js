...
export function initializeSettings(scrobbler, player, api, ui) {
    ...
    if (languageSelect && window.i18n) {
        ...
        console.log('[settings] Initializing language selector with:', currentLang);
        ...
        console.warn('[settings] window.i18n not found!');
    }
    ...
    // Crossfade Settings
    // ========================================
    const crossfadeEnable = document.getElementById('crossfade-enable');
    ...
    const crossfadeAuto = document.getElementById('crossfade-auto');
    const crossfadePreviewBtn = document.getElementById('crossfade-preview-btn');

    if (crossfadeEnable && crossfadeDuration && crossfadeCurve && crossfadeAuto) {
        // Load current settings
        const currentSettings = crossfadeSettings.get();
        crossfadeEnable.checked = currentSettings.enabled;
        crossfadeDuration.value = currentSettings.duration;
        crossfadeDurationValue.textContent = `${currentSettings.duration / 1000}s`;
        crossfadeCurve.value = currentSettings.curve;
        crossfadeAuto.checked = currentSettings.autoCrossfade;

        // Enable/Disable crossfade
        crossfadeEnable.addEventListener('change', (e) => {
            crossfadeSettings.set({ enabled: e.target.checked });
            
            if (e.target.checked) {
                showNotification('Crossfade enabled. Transitions will be smooth! 🎵');
            } else {
                showNotification('Crossfade disabled');
            }
        });
        ...
        crossfadeDuration.addEventListener('input', (e) => {
            ...
            crossfadeDurationValue.textContent = `${duration / 1000}s`;
            crossfadeSettings.setDuration(duration);
        });

        // Fade curve select
        crossfadeCurve.addEventListener('change', (e) => {
            crossfadeSettings.setCurve(e.target.value);
            showNotification(`Fade curve changed to: ${e.target.value}`);
        });

        // Auto crossfade toggle
        crossfadeAuto.addEventListener('change', (e) => {
            crossfadeSettings.set({ autoCrossfade: e.target.checked });
        });

        // Preview button
        if (crossfadePreviewBtn) {
            crossfadePreviewBtn.addEventListener('click', async () => {
                if (!player.audio.src || player.audio.paused) {
                    showNotification('Please play a track first to preview crossfade 🎧');
                    return;
                }
                
                showNotification('Previewing crossfade...');
                await player.previewCrossfade();
            });
        }
    }
