...
export class UIRenderer {
    ...
    async renderHomePage() {
        ...
        const infiniteStreamBtn = document.getElementById('start-infinite-stream-btn');
        const infiniteStreamBanner = document.getElementById('infinite-stream-banner');

        const startInfiniteStream = async () => {
            const seeds = await this.getSeeds();
            const trackSeeds = seeds.slice(0, 5);
            
            if (trackSeeds.length === 0) {
                showNotification('Please listen to or like some tracks first to start Endless Mix.');
                return;
            }
            
            showNotification('Starting Endless Mix...');
            try {
                let recs = await this.api.getRecommendedTracksForPlaylist(trackSeeds, 20, { skipCache: true });
                recs = await this.filterUserContent(recs, 'track');
                
                if (recs && recs.length > 0) {
                    this.player.clearQueue();
                    this.player.isWaveMode = true;
                    // Keep track of all played tracks to prevent duplicates
                    this.player.playedWaveTracks = new Set(recs.map(t => t.id));
                    this.player.setQueue(recs);
                    this.player.playAtIndex(0);
                } else {
                    showNotification('Could not find enough tracks to start.');
                }
            } catch (e) {
                console.error('Infinite stream error:', e);
                showNotification('Failed to start Endless Mix.');
            }
        };

        if (infiniteStreamBtn) infiniteStreamBtn.onclick = startInfiniteStream;
        if (infiniteStreamBanner) infiniteStreamBanner.onclick = (e) => {
            // Prevent double-firing if the button itself was clicked
            if (e.target.closest('#start-infinite-stream-btn')) return;
            startInfiniteStream();
        };
            ...
            const seeds = await this.getSeeds();
            this.updateEndlessMixBannerState();
            await Promise.all([
    ...
    async renderExplorePage() {
        ...
        try {
            ...
            const GENRES = [
                { id: 'hip_hop', name: 'Hip Hop / Rap', i18n: 'hip-hop-rap' },
                { id: 'pop', name: 'Pop', i18n: 'pop'  },
                { id: 'rock', name: 'Rock', i18n: 'rock'  },
                { id: 'electronic', name: 'Electronic', i18n: 'electronic'  },
                { id: 'country', name: 'Country', i18n: 'country'  },
                { id: 'jazz', name: 'Jazz', i18n: 'jazz'  },
                { id: 'classical', name: 'Classical', i18n: 'classical'  },
                { id: 'latin', name: 'Latin', i18n: 'latin'  },
                { id: 'reggae', name: 'Reggae / Dancehall', i18n: 'reggae'  },
                { id: 'blues', name: 'Blues', i18n: 'blues'  },
                { id: 'soundtrack', name: 'Soundtrack', i18n: 'soundtrack'  },
                { id: 'alternative', name: 'Alternative', i18n: 'alternative'  },
            ];

            if (GENRES.length > 0) {
                const genresSection = document.createElement('section');
                genresSection.className = 'content-section';
                genresSection.innerHTML = `<h2 class="section-title" data-i18n="nav-genres">${window.i18n ? window.i18n.t('nav-genres') : 'Genres'}</h2>`;
                ...
                genresGrid.innerHTML = GENRES.map(
                    (genre) => `
                    <div class="card genre-card" data-genre-id="${genre.id}" data-genre-name="${escapeHtml(genre.name)}" data-genre-i18n="${genre.i18n}" style="cursor: pointer; background: var(--secondary); padding: 0.6rem 1rem; border-radius: var(--radius); border: 1px solid var(--border);">
                        <h3 style="margin: 0; font-size: 0.875rem; font-weight: 600;" data-i18n="${genre.i18n}">${window.i18n ? window.i18n.t(genre.i18n) : escapeHtml(genre.name)}</h3>
                    </div>
                `
                ).join('');
                ...
                genresGrid.querySelectorAll('.genre-card').forEach((card) => {
                    card.addEventListener('click', () => {
                        this.renderGenrePage(card.dataset.genreId, card.dataset.genreName, card.dataset.genreI18n);
                    });
                });
            }

            if (data.top_albums && data.top_albums.length > 0) {
                this.renderExploreSection(container, window.i18n ? window.i18n.t('nav-trending-albums') : 'Trending Albums', data.top_albums, 'album', 'nav-trending-albums');
            }

            if (data.top_tracks && data.top_tracks.length > 0) {
                this.renderExploreSection(container, window.i18n ? window.i18n.t('nav-trending-tracks') : 'Trending Tracks', data.top_tracks, 'track', 'nav-trending-tracks');
            }

            if (data.featured_playlists && data.featured_playlists.length > 0) {
                this.renderExploreSection(container, window.i18n ? window.i18n.t('nav-featured-playlists') : 'Featured Playlists', data.featured_playlists, 'playlist', 'nav-featured-playlists');
            }
                    ...
                    if (section.items && section.items.length > 0) {
                        ...
                        if (type) {
                            const sectionTitleMap = {
                                "The Hits": "section-the-hits",
                                "New Tracks": "section-new-tracks",
                                "New Albums": "section-new-albums",
                                "Spotlighted Uploads": "section-spotlighted-uploads",
                                "From our editors": "section-from-our-editors"
                            };
                            
                            const fallbackTitle = section.title;
                            const i18nKey = sectionTitleMap[fallbackTitle];
                            const translatedTitle = (window.i18n && i18nKey) ? window.i18n.t(i18nKey) : fallbackTitle;

                            this.renderExploreSection(container, translatedTitle, section.items, type, i18nKey);
                        }
                    }
                });
            }
    ...
    renderExploreSection(container, title, items, type, i18nKey = null) {
        const section = document.createElement('section');
        section.className = 'content-section';
        section.innerHTML = `<h2 class="section-title"${i18nKey ? ` data-i18n="${i18nKey}"` : ''}>${title}</h2>`;
    ...
    async renderGenrePage(genreId, genreName, genreI18n) {
        const container = document.getElementById('explore-grid');
        if (!container) return;
        ...
        container.innerHTML = `
            <div style="margin-bottom: 1.5rem; display: flex; align-items: center; gap: 1rem;">
                <button class="btn-secondary explore-back-btn" style="display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    <span data-i18n="btn-back">${window.i18n ? window.i18n.t('btn-back') : 'Back'}</span>
                </button>
                <h2 class="section-title" style="margin: 0;" ${genreI18n ? `data-i18n="${genreI18n}"` : ''}>${genreI18n && window.i18n ? window.i18n.t(genreI18n) : escapeHtml(genreName)}</h2>
            </div>
                ...
                data.sections.forEach((section) => {
                    if (section.items && section.items.length > 0) {
                        ...
                        if (type) {
                            ...
                            this.renderExploreSection(contentContainer, translatedTitle, section.items, type, i18nKey);
                        }
                    }
                });
            }
    ...
    updateEndlessMixBannerState() {
        const banner = document.getElementById('infinite-stream-banner');
        const btn = document.getElementById('start-infinite-stream-btn');
        if (!banner || !btn) return;

        if (this.player.isWaveMode) {
            banner.classList.add('is-active');
            
            const isPlaying = !this.player.audio.paused && this.player.audio.src;
            
            if (isPlaying) {
                banner.classList.remove('is-paused');
                btn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="margin-right: 6px;"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg> Playing Endless Mix`;
                
                const iconContainer = banner.querySelector('.endless-mix-icon');
                if (iconContainer) {
                    iconContainer.innerHTML = `
                        <div class="endless-eq">
                ...
                btn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 6px;"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg> Resume Endless Mix`;
                
                const iconContainer = banner.querySelector('.endless-mix-icon');
                if (iconContainer) {
                    iconContainer.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>`;
                }
            }
        } else {
