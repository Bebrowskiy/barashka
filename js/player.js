...
export class Player {
    constructor(audioElement, api, quality = 'HI_RES_LOSSLESS') {
        ...
        this.isWaveMode = false; // Sonic Stream Mode
        this.isFetchingWave = false; // Prevent concurrent fetches
    ...
    async playTrackFromQueue(startTime = 0, recursiveCount = 0) {
        ...
        // Endless Mix logic: if we are near the end of the queue, fetch more tracks
        if (this.isWaveMode && !this.isFetchingWave && this.currentQueueIndex >= currentQueue.length - 4) {
            this.isFetchingWave = true;
            try {
                // Determine seed tracks based on recently played in the queue
                const seedTracks = currentQueue.slice(Math.max(0, this.currentQueueIndex - 4), this.currentQueueIndex + 1);
                
                const recs = await this.api.getRecommendedTracksForPlaylist(seedTracks, 20, { skipCache: true });
                
                if (recs && recs.length > 0) {
                    if (!this.playedWaveTracks) {
                        this.playedWaveTracks = new Set(currentQueue.map(t => t.id));
                    }
                    
                    // Filter duplicates against the history of the entire stream session
                    const newTracks = recs.filter((rt) => !this.playedWaveTracks.has(rt.id));
                    
                    if (newTracks.length > 0) {
                        newTracks.forEach(t => this.playedWaveTracks.add(t.id));
                        this.addNextToQueue(newTracks);
                    } else {
                        // If all recommendations were duplicates, try another set of seeds from slightly further back
                        const fallbackSeedTracks = currentQueue.slice(Math.max(0, this.currentQueueIndex - 8), Math.max(0, this.currentQueueIndex - 3));
                        if (fallbackSeedTracks.length > 0) {
                            const fallbackRecs = await this.api.getRecommendedTracksForPlaylist(fallbackSeedTracks, 20, { skipCache: true });
                            if (fallbackRecs) {
                                const fallbackNewTracks = fallbackRecs.filter(rt => !this.playedWaveTracks.has(rt.id));
                                if (fallbackNewTracks.length > 0) {
                                    fallbackNewTracks.forEach(t => this.playedWaveTracks.add(t.id));
                                    this.addNextToQueue(fallbackNewTracks);
                                }
                            }
                        }
                    }
                ...
                console.error('Failed to fetch Endless Mix tracks:', err);
            } finally {
                this.isFetchingWave = false;
            }
        }
    ...
    async playNext(recursiveCount = 0) {
        ...
        let nextIndex = this.currentQueueIndex;
        ...
        // === CROSSFADE LOGIC ===
        // Проверяем, можем ли мы использовать crossfade
        if (this.canCrossfadeNow()) {
            console.log('[PlayNext] Attempting crossfade transition...');
            
            const success = await this.executeCrossfadeTransition(nextIndex);
            
            if (success) {
                console.log('[PlayNext] Crossfade transition successful');
                return;
            }
            
            // Если crossfade не удался, продолжаем со стандартным переключением
            console.log('[PlayNext] Crossfade failed, falling back to standard transition');
        }

        // Стандартное переключение (fallback)
    ...
    clearQueue() {
        ...
        this.isWaveMode = false; // Disable wave mode when clearing queue
        this.preloadCache.clear();
        this.saveQueueState();
    }
    ...
    /**
     * Загрузка настроек crossfade
     */
    async loadCrossfadeSettings() {
        const settings = crossfadeSettings.get();
        this.crossfadeEnabled = settings.enabled;
        this.crossfadeDuration = settings.duration;
        this.crossfadeCurve = settings.curve;
        this.crossfadeAuto = settings.autoCrossfade;
        
        console.log('[Player] Crossfade settings loaded:', settings);
    }

    /**
     * Проверка возможности crossfade в данный момент
     */
    canCrossfadeNow() {
        if (!this.crossfadeEnabled || !this.crossfadeAuto) {
            return false;
        }
        
        const timeRemaining = this.audio.duration - this.audio.currentTime;
        const durationSeconds = this.crossfadeDuration / 1000;
        
        // Требуем минимум 1 секунду трека после завершения crossfade
        return timeRemaining >= (durationSeconds + 1);
    }

    /**
     * Выполнение crossfade перехода к следующему треку
     * @param {number} nextIndex - Индекс следующего трека в очереди
     * @returns {Promise<boolean>} - Успешность перехода
     */
    async executeCrossfadeTransition(nextIndex) {
        const currentQueue = this.shuffleActive ? this.shuffledQueue : this.queue;
        
        if (nextIndex < 0 || nextIndex >= currentQueue.length) {
            console.log('[Crossfade] Invalid next index');
            return false;
        }

        const nextTrack = currentQueue[nextIndex];
        
        // Проверка на доступность трека
        const { contentBlockingSettings } = await import('./storage.js');
        if (nextTrack?.isUnavailable || contentBlockingSettings.shouldHideTrack(nextTrack)) {
            console.log('[Crossfade] Next track is unavailable or blocked');
            return false;
        }

        console.log(`[Crossfade] Starting ${this.crossfadeDuration}ms transition to: ${nextTrack.title}`);

        try {
            // Получаем URL следующего трека
            const nextStreamUrl = await this.api.getStreamURL(nextTrack.id, this.quality);
            
            // Создаём новый Audio элемент
            const nextAudio = new Audio();
            nextAudio.preload = 'auto';
            nextAudio.src = nextStreamUrl;
            
            // Ждём готовности трека к воспроизведению
            await new Promise((resolve, reject) => {
                const timeout = setTimeout(() => {
                    reject(new Error('Next track load timeout'));
                }, 15000);
                ...
                const errorHandler = (e) => {
                    clearTimeout(timeout);
                    nextAudio.removeEventListener('canplay', canPlayHandler);
                    reject(new Error(`Next track load error: ${e.message}`));
                };
                
                nextAudio.addEventListener('canplay', canPlayHandler);
                nextAudio.addEventListener('error', errorHandler);
            });

            // Запускаем crossfade через менеджер
            const success = await this.crossfadeManager.transitionTo(
                nextAudio,
            ...
            if (success) {
                ...
                console.log('[Crossfade] Transition completed successfully');
                return true;
            }
     ...
     */
    async previewCrossfade() {
        if (!this.audio.src || this.audio.paused) {
            showNotification('Please play a track first to preview crossfade');
            return;
        }
        
        const settings = crossfadeSettings.get();
        await this.crossfadeManager.preview(settings.duration, settings.curve);
    ...
    /**
     * Получить статус crossfade
     */
    getCrossfadeStatus() {
        return {
            enabled: this.crossfadeEnabled,
            isActive: this.crossfadeManager?.isActive || false,
            duration: this.crossfadeDuration,
            curve: this.crossfadeCurve,
            ...this.crossfadeManager?.getStatus(),
        };
    }
}
