...
/**
 * Copy text to clipboard (works in HTTP and HTTPS)
 */
...
function fallbackCopy(text) {
    ...
    try {
        ...
        // Last resort: prompt
        const result = prompt('Press Ctrl+C to copy this link:', text);
        if (result !== null) {
            showNotification('Link ready to copy');
        }
    }
...
export function initializePlayerEvents(player, audioPlayer, scrobbler, ui) {
    ...
    audioPlayer.addEventListener('play', () => {
        ...
        if (ui && ui.updateEndlessMixBannerState) ui.updateEndlessMixBannerState();
    });
...
export async function handleTrackAction(
    ...
    if (action === 'add-to-queue') {
        ...
        const trackId = item.id || item.uuid;
        const url = getShareUrl(storedHref ? storedHref : `/track/${trackId}`);
        const shareText = `Listen to ${item.title || 'this track'} on Barashka Music`;

        console.log('[Share] URL:', url);

        // Try Web Share API first (works on mobile and HTTPS)
        if (navigator.share) {
            navigator.share({
                title: 'Barashka Music',
                text: shareText,
                url: url
            })
            .then(() => console.log('[Share] Shared successfully'))
            .catch((err) => {
                console.log('[Share] Share cancelled or failed:', err);
                // If cancelled, copy to clipboard as fallback
                copyToClipboard(url);
            });
        } else {
