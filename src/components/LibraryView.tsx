...
import { Play, Pause, Heart, Clock, MoreHorizontal, Shuffle, LayoutGrid, List, Plus, Menu } from 'lucide-react';
...
export default function LibraryView({ onMenuClick }: { onMenuClick?: () => void }) {
    const { currentTrack, isPlaying, playTrack, likedTracks, toggleLike, openArtist, openPlaylist, setIsCreatePlaylistOpen, playTrackWithQueue } = usePlayer();
    const { t } = useI18n();
                                ...
                                {savedTracks.length === 0 ? (
                                    ...
                                    <>
                                        <div className="flex items-center gap-4 mb-6">
                                            <button
                                                onClick={() => { if(savedTracks.length > 0) playTrackWithQueue(savedTracks[0], savedTracks, 0) }}
